...
%{
    if(uvms.abs_diff_jlmin<uvms.abs_diff_jlmax)
        %Qui devo considerare  il limite minimo
        
        %Questo if server per evitare il rischio che la diff sia uguale a 0
        %E che quindi successivamente 1/diff=infinito
        if(uvms.abs_diff_jlmin>0.0001)
            uvms.xdot.jl(i,1)=-1/uvms.diff_jlmin(i,1);
        else 
            uvms.xdot.jl(i,1)=-10000*sign(diff_jlmin(i,1));
        end
        
    else
        %Qui devo considerare il limite massimo
        if(uvms.abs_diff_jlmax>0.01)
            uvms.xdot.jl(i,1)=-1/uvms.diff_jlmax(i,1);
        else 
            uvms.xdot.jl(i,1)=-100*sign(uvms.diff_jlmax(i,1));
        end
